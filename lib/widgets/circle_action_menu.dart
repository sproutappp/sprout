    ...
    if (action == 'invite') {
      ...
      final confirmed = await showDialog<bool>(
        ...
        builder: (dialogContext) => AlertDialog(
          ...
          content: Text(
            'This permanently deletes the circle and its memories. This cannot be undone.',
            style: GoogleFonts.manrope(
              fontSize: 13,
      ...
      try {
        ...
        onChanged?.call();
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('${circle.name} deleted.')),
        );
      } catch (_) {
        if (!context.mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Couldn't delete the circle. Try again.")),
        );
      }
    }
  }
...
class _CircleInvitePickerState extends State<_CircleInvitePicker> {
  ...
  Future<void> _send() async {
    ...
    try {
      final count = await CirclesRepository.sendCircleInvites(circleId: widget.circle.id, userIds: _selected.toList());
      if (!mounted) return;
      Navigator.pop(context);
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('$count invite${count == 1 ? '' : 's'} sent.')));
    } catch (_) {
      if (!mounted) return;
      setState(() => _sending = false);
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Couldn't send invites. Try again.")));
    }
  }
                ...
                itemBuilder: (_, index) {
                  ...
                  return GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(color: selected ? AppTheme.primaryGreenGlow : AppTheme.surfaceVariantDark, borderRadius: BorderRadius.circular(14), border: Border.all(color: selected ? AppTheme.primaryGreen.withAlpha(100) : AppTheme.outline)),
                      child: Row(children: [_Avatar(url: user.avatarUrl), const SizedBox(width: 12), Expanded(child: Text(user.displayName, style: GoogleFonts.manrope(fontSize: 14, fontWeight: FontWeight.w600, color: AppTheme.textPrimary))), Icon(selected ? Icons.check_circle_rounded : Icons.radio_button_unchecked_rounded, color: selected ? AppTheme.primaryGreen : AppTheme.textDisabled, size: 22)]),
                    ),
                  );
                },
              ),
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: _sending
                  ...
                  : Text(
                      _selected.isEmpty ? 'Send Invite' : 'Send Invite (${_selected.length})',
                      style: GoogleFonts.manrope(
                        fontSize: 15,
                        fontWeight: FontWeight.w700,
                        color: Colors.black,
