...
class EditMemoryScreen extends StatefulWidget {
  ...
  const EditMemoryScreen({super.key, required this.memoryId});
  @override
...
class _EditMemoryScreenState extends State<EditMemoryScreen> {
  ...
  @override
  void dispose() { _titleController.dispose(); _captionController.dispose(); _locationController.dispose(); super.dispose(); }
  ...
  Future<void> _load() async {
    try {
      ...
      final people = circleIds.isEmpty ? <Profile>[] : await CirclesRepository.fetchMembersForCircles(circleIds);
      final tagged = await MemoryPeopleRepository.fetchForMemory(widget.memoryId);
      if (!mounted) return;
      final stored = (memory['caption'] as String? ?? '').trim();
      final separator = stored.indexOf(' — ');
      _titleController.text = separator > 0 ? stored.substring(0, separator) : stored;
      _captionController.text = separator > 0 ? stored.substring(separator + 3) : '';
      ...
      setState(() { _circles = circles; _people = people; _loading = false; });
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _refreshPeople() async {
    if (_isPublic || _selectedCircleIds.isEmpty) {
      setState(() { _people = []; _taggedPeople.clear(); });
      return;
    }
    try {
      final people = await CirclesRepository.fetchMembersForCircles(_selectedCircleIds.toList());
      final valid = people.map((p) => p.id).toSet();
      if (!mounted) return;
      setState(() { _people = people; _taggedPeople.removeWhere((id) => !valid.contains(id)); });
    } catch (_) {}
  }

  Future<void> _save() async {
    setState(() => _error = null);
    if (!_isPublic && _selectedCircleIds.isEmpty) { setState(() => _error = 'Choose at least one circle for a private memory.'); return; }
    setState(() => _saving = true);
    try {
      final combined = [_titleController.text.trim(), _captionController.text.trim()].where((s) => s.isNotEmpty).join(' — ');
      await MemoryEditRepository.updateMemory(memoryId: widget.memoryId, caption: combined, location: _locationController.text, isPublic: _isPublic, circleIds: _selectedCircleIds.toList());
      await MemoryPeopleRepository.replaceForMemory(memoryId: widget.memoryId, personIds: _isPublic ? const [] : _taggedPeople.toList());
      if (mounted) Navigator.pop(context, true);
    } catch (_) {
      if (mounted) setState(() { _saving = false; _error = "Couldn't save changes. Please try again."; });
    }
  }
              ...
              : ListView(
                  ...
                  children: [
                    ...
                    SwitchListTile.adaptive(
                      ...
                      onChanged: (value) async { setState(() { _isPublic = value; if (value) _selectedCircleIds.clear(); }); await _refreshPeople(); },
                    ),
                    if (!_isPublic) ...[
                      const SizedBox(height: 8),
                      Text('Circles', style: GoogleFonts.manrope(color: AppTheme.textPrimary, fontSize: 13, fontWeight: FontWeight.w800)),
                      const SizedBox(height: 8),
                      if (_circles.isEmpty) const Text('Create a circle first.', style: TextStyle(color: AppTheme.textMuted))
                      else ..._circles.map((circle) => CheckboxListTile(
                        contentPadding: EdgeInsets.zero,
                        value: _selectedCircleIds.contains(circle.id),
                        activeColor: AppTheme.primaryGreen,
                        title: Text(circle.name, style: const TextStyle(color: AppTheme.textPrimary)),
                        onChanged: (checked) async { setState(() { if (checked == true) _selectedCircleIds.add(circle.id); else _selectedCircleIds.remove(circle.id); }); await _refreshPeople(); },
                      )),
                      ...
                      const SizedBox(height: 8),
                      if (_people.isEmpty) const Text('No people available for the selected circles.', style: TextStyle(color: AppTheme.textMuted, fontSize: 12))
                      else ..._people.map((person) => CheckboxListTile(
                        contentPadding: EdgeInsets.zero,
                        ...
                        secondary: CircleAvatar(
                          ...
                          child: person.avatarUrl == null ? Text(person.displayName.isEmpty ? '?' : person.displayName.substring(0, 1).toUpperCase()) : null,
                        ),
                        onChanged: (checked) => setState(() { if (checked == true) _taggedPeople.add(person.id); else _taggedPeople.remove(person.id); }),
                      )),
                    ],
                    if (_error != null) ...[const SizedBox(height: 10), Text(_error!, textAlign: TextAlign.center, style: const TextStyle(color: AppTheme.error, fontSize: 12))],
                    const SizedBox(height: 18),
                    SizedBox(height: 52, child: FilledButton(
                      ...
                      child: _saving ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.black)) : const Text('Save Changes', style: TextStyle(fontWeight: FontWeight.w800)),
                    )),
                  ],
                ),
    );
