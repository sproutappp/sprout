...
class _CreateMemoryScreenV2State extends State<CreateMemoryScreenV2> {
  ...
  @override
  void dispose() { _title.dispose(); _caption.dispose(); _location.dispose(); super.dispose(); }
  ...
  Future<void> _loadPeople() async {
    if (_public || _circleIds.isEmpty) {
      if (mounted) setState(() { _people = []; _taggedPeople.clear(); _loadingPeople = false; });
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      setState(() { _people = people; _taggedPeople.removeWhere((id) => !ids.contains(id)); _loadingPeople = false; });
    } catch (_) { if (mounted) setState(() { _people = []; _loadingPeople = false; }); }
  }

  Future<void> _pick(ImageSource source) async {
    try {
      final picked = await _picker.pickImage(source: source, maxWidth: 2048, imageQuality: 88);
      if (picked != null && mounted) setState(() { _image = File(picked.path); _error = null; });
    } catch (_) { if (mounted) setState(() => _error = "Couldn't access your photos."); }
  }
  ...
  Future<void> _locate() async {
    ...
    try {
      ...
      final parts = <String>[if ((p.name ?? '').isNotEmpty) p.name!, if ((p.locality ?? '').isNotEmpty) p.locality!, if ((p.administrativeArea ?? '').isNotEmpty) p.administrativeArea!];
  ...
  Future<void> _save() async {
    ...
    if (_image == null) { setState(() => _error = 'Add a photo before saving.'); return; }
    if (!_public && _circleIds.isEmpty) { setState(() => _error = 'Choose at least one circle, or make it public.'); return; }
    ...
    try {
      ...
      final memory = await MemoriesRepository.addMemory(
        file: _image!, caption: combined.isEmpty ? null : combined,
        location: _location.text.trim().isEmpty ? null : _location.text.trim(),
        isPublic: _public, circleIds: _public ? const [] : _circleIds.toList(),
      );
      if (!_public && _taggedPeople.isNotEmpty) {
        await MemoryPeopleRepository.replaceForMemory(memoryId: memory.id, personIds: _taggedPeople.toList());
      }
      if (mounted) Navigator.pop(context, true);
    } catch (_) {
      if (mounted) setState(() { _saving = false; _error = "Couldn't save this memory. Please try again."; });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [TextButton(onPressed: _saving ? null : _save, child: Text('Save', style: GoogleFonts.manrope(fontWeight: FontWeight.w800)))],
      ),
      body: _loading
          ...
          : ListView(
              ...
              children: [
                ...
                SwitchListTile.adaptive(
                  ...
                  value: _public, activeColor: AppTheme.primaryGreen,
                  onChanged: (v) async { setState(() { _public = v; if (v) _circleIds.clear(); }); await _loadPeople(); },
                ),
                if (!_public) ...[
                  ...
                  ..._circles.map((c) => CheckboxListTile(
                    contentPadding: EdgeInsets.zero, activeColor: AppTheme.primaryGreen,
                    title: Text(c.name, style: const TextStyle(color: AppTheme.textPrimary)),
                    value: _circleIds.contains(c.id),
                    onChanged: (v) async { setState(() { if (v == true) _circleIds.add(c.id); else _circleIds.remove(c.id); }); await _loadPeople(); },
                  )),
                  ...
                  const SizedBox(height: 6),
                  if (_loadingPeople) const LinearProgressIndicator(minHeight: 2, color: AppTheme.primaryGreen)
                  else ..._people.map((p) => CheckboxListTile(
                    contentPadding: EdgeInsets.zero, activeColor: AppTheme.primaryGreen,
                    title: Text(p.displayName, style: const TextStyle(color: AppTheme.textPrimary)),
                    value: _taggedPeople.contains(p.id),
                    onChanged: (v) => setState(() { if (v == true) _taggedPeople.add(p.id); else _taggedPeople.remove(p.id); }),
                  )),
                ],
                if (_error != null) ...[const SizedBox(height: 12), Text(_error!, textAlign: TextAlign.center, style: const TextStyle(color: AppTheme.error, fontSize: 12))],
                const SizedBox(height: 18),
                SizedBox(height: 52, child: FilledButton(
                  ...
                  child: _saving ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.black)) : const Text('Save Memory', style: TextStyle(fontWeight: FontWeight.w800)),
                )),
              ],
            ),
    );
  ...
  Widget _photoPicker() => Container(
    ...
    child: _image == null
        ? Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            const SizedBox(height: 14),
            Wrap(spacing: 10, children: [OutlinedButton.icon(onPressed: () => _pick(ImageSource.camera), icon: const Icon(Icons.camera_alt_outlined), label: const Text('Camera')), OutlinedButton.icon(onPressed: () => _pick(ImageSource.gallery), icon: const Icon(Icons.photo_library_outlined), label: const Text('Gallery'))]),
          ])
        : Stack(fit: StackFit.expand, children: [Image.file(_image!, fit: BoxFit.cover), Positioned(top: 12, right: 12, child: IconButton.filled(onPressed: () => setState(() => _image = null), icon: const Icon(Icons.close_rounded))) ]),
  );
  ...
  Widget _locationField() => TextField(
    ...
    decoration: InputDecoration(
      labelText: 'Location', labelStyle: const TextStyle(color: AppTheme.textMuted), hintText: _locating ? 'Finding location...' : 'Optional location', hintStyle: const TextStyle(color: AppTheme.textDisabled),
  ...
  Widget _field(String label, TextEditingController controller, String hint, {int maxLines = 1}) => TextField(
    ...
    decoration: InputDecoration(labelText: label, labelStyle: const TextStyle(color: AppTheme.textMuted), hintText: hint, hintStyle: const TextStyle(color: AppTheme.textDisabled), filled: true, fillColor: AppTheme.cardDark, border: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: const BorderSide(color: AppTheme.outline)), enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: const BorderSide(color: AppTheme.outline)), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: const BorderSide(color: AppTheme.primaryGreen))),
  );
}
